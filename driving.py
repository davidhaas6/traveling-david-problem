...
def distance(coord1, coord2):
    ...
    driving_distance = (
        distance_miles * 1.4
    )  # 1.4 accounts for road-length. see https://circuity.org/
...
def distance_with_uncertainty(coord1, coord2, uncertainty_factor=0.1):
    ...
    sampled_distance = np.random.normal(
        base_distance, base_distance * uncertainty_factor
    )
...
def tour_length(cities, tour):
    return sum(
        distance(cities[tour[i]], cities[tour[i + 1]]) for i in range(len(tour) - 1)
    )


def tour_length_with_uncertainty(cities, tour):
    return sum(
        distance_with_uncertainty(cities[tour[i]], cities[tour[i + 1]])
...
def simulated_annealing(cities, initial_temp, cooling_rate, seed=None):
    ...
    while temp > 1:
        ...
        if new_distance < current_distance or random.random() < math.exp(
            (current_distance - new_distance) / temp
        ):
...
def genetic_algorithm(cities, population_size, generations, mutation_rate):
    population = [
        random.sample(range(len(cities)), len(cities)) for _ in range(population_size)
    ]
...
def simulated_annealing_with_uncertainty(
    cities, initial_temp, cooling_rate, num_iterations=100
):
    results = []
...
if __name__ == "__main__":
    cities = [
        ("Richmond", (37.5407246, -77.4360481)),
        ("NOVA", (38.9338676, -77.1772604)),
        ...
        ("Maine", (43.661471, -70.2553259)),
        ("Seattle", (47.6062095, -122.3320708)),
        ("Los Angeles", (34.0522342, -118.2436849)),
    ]

    city_names, cities_coordinates = zip(*cities)

    def print_results(algorithm_name, length, tour):
        # start the trip with nova
        nova_tour_idx = tour.index(city_names.index("NOVA"))
        tour = tour[nova_tour_idx:] + tour[:nova_tour_idx]
        print(f"{algorithm_name}: {int(length)} mi\t\t\t{[city_names[i][:4] for i in tour]}")

    nn_tour = nearest_neighbor(cities_coordinates)
    print_results("Nearest Neighbor", tour_length(cities_coordinates, nn_tour), nn_tour)
    
    
    mst_tour = mst_approximation(cities_coordinates)
    print_results("MST Approximation", tour_length(cities_coordinates, mst_tour), mst_tour)
    
    sa_tour = simulated_annealing(cities_coordinates, initial_temp=10000, cooling_rate=0.995)
    print_results("Simulated Annealing", tour_length(cities_coordinates, sa_tour), sa_tour)
    
    ga_population = genetic_algorithm(cities_coordinates, population_size=10, generations=100, mutation_rate=0.1)
    for i, ga_tour in enumerate(ga_population[:5]):
        print_results(f"Genetic Algorithm #{i+1}", tour_length(cities_coordinates, ga_tour), ga_tour)
        
    
    mc_solutions = monte_carlo_simulation(cities_coordinates, num_simulations=20000)
    # sort mc_solutions by tour length
    mc_solutions.sort(key=lambda x: tour_length(cities_coordinates, x))
    for i, mc_tour in enumerate(mc_solutions[:5]):
        print_results(f"Monte Carlo Sim #{i+1}", tour_length(cities_coordinates, mc_tour), mc_tour)
    ...
    print_results("Best Tour", best_distance, best_tour)
